									   ...
									   const QString& ssPrice, const QString& bbQty, const QString& bsQty, const QString& sbQty, const QString& ssQty)
{
	...
	if (itemList.size() == 0){
		...
		addItem(nRow, 6, bbPrice);
		addItem(nRow, 7, ssPrice);	
		addItem(nRow, 8, bbQty);
		addItem(nRow, 9, bsQty);
		addItem(nRow, 10, sbQty);
		...
		updateItemPrice(nSelRow, 6, bbPrice);
		updateItemPrice(nSelRow, 7, ssPrice);
		updateItemVolume(nSelRow, 8, bbQty);
		updateItemVolume(nSelRow, 9, bsQty);
		updateItemVolume(nSelRow, 10, sbQty);
