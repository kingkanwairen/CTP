...
{
	connect(this, SIGNAL(itemClicked(QTreeWidgetItem*, int)), this, SLOT(doSelectItem(QTreeWidgetItem*, int)));
...
void USTPositionWidget::doSelectItem(QTreeWidgetItem* item, int column)
{
	
	if(column == 1 || column == 2){
		QString szQty = item->text(column);
		int posQty = szQty.toInt();
		if(posQty > 0){
			QString ins = item->text(0);
			int bs = (column == 1) ? 1 : 0;
			emit onUpdatePosition(ins, bs, posQty);
		}
	}
}
