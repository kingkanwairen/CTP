...
{	
	...
	mMdOrderQtyLabel = new QLabel(tr("最小挂单量:"));
	mMdOrderQtySpinBox = new QSpinBox;
	mMdOrderQtySpinBox->setRange(1, 10000);
	mMdOrderQtySpinBox->setSingleStep(1);
	mMdOrderQtySpinBox->setValue(10);
	mMdOrderQtyLabel->setBuddy(mMdOrderQtySpinBox);
	...
	mMdOrderQtyCheckBox = new QCheckBox(tr("挂单限量"));
	...
	vBox->addWidget(mMdOrderQtyLabel, 7, 3, 1, 1);
	vBox->addWidget(mMdOrderQtySpinBox, 7, 4, 1, 2);
	...
	vBox->addWidget(mMdOrderQtyCheckBox, 9, 3, 1, 1);
...
{	
	...
	connect(this, SIGNAL(onCreateNewOrder(const QString&, const QString&, const QString&, const QString&, const QString&, const QString&, const double&, 
		const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, bool, bool, bool, bool, bool, bool)), 
...
void USTParamDialog::createOrder()
{
	...
	if(firstName.isEmpty() || secondName.isEmpty()){
		...
		int mdOrderQty = mMdOrderQtySpinBox->value();
		int cyclePrice = mCycleSpinBox->value();
		bool isOrderQtyCheck = mMdOrderQtyCheckBox->isChecked();
		...
		emit onCreateNewOrder(firstName, secondName, style, bs, offset, hedge, price, qty, firtTime, secTime, firstPrice, secondPrice, referTickNum,
			superTickNum, cyclePrice, actionReferTickNum, actionSuperSlipNum, mdOrderQty, isOrderQtyCheck, isOppoentPriceCheck, isFirstTimeCheck, isSecondTimeCheck,
