...
{	
	...
	foreach(QString ins, inss){
		...
		if (items.size() == 2){
			...
			if(iter == mInss.end()){
				doUSTPSpeComplexMd(oneIns, "0.0", "0", "0.0", "0", "0", "0.0");
			...
			if(iter == mInss.end()){
				doUSTPSpeComplexMd(secIns, "0.0", "0", "0.0", "0", "0", "0.0");
...

{
	...
	if (itemList.size() == 0){
		...
		QString vol = QString::number(USTPMutexId::getActionNum(selIns)); 	 
		addItem(nRow, 7, vol);
	}
	else{
		...
		QString vol = QString::number(USTPMutexId::getActionNum(selIns));
		updateItemVolume(nSelRow, 7, vol);	
	}
}
