...
{	
	...
	mHeaders << DEPTH_WIDGET_TAB_HEAD_0 << DEPTH_WIDGET_TAB_HEAD_1 << DEPTH_WIDGET_TAB_HEAD_2 << DEPTH_WIDGET_TAB_HEAD_3
		...
		<< DEPTH_WIDGET_TAB_HEAD_8 << DEPTH_WIDGET_TAB_HEAD_9 << DEPTH_WIDGET_TAB_HEAD_10 << DEPTH_WIDGET_TAB_HEAD_11;
									   ...
									   const QString& ssPrice, const QString& bbQty, const QString& bsQty, const QString& sbQty, const QString& ssQty)
{
	...
	if (itemList.size() == 0){
		...
		QString vol = QString::number(USTPMutexId::getActionNum(key)); 	 
		addItem(nRow, 3, vol);
		addItem(nRow, 4, bsPrice);
		addItem(nRow, 5, bsQty);
		addItem(nRow, 6, sbPrice);	
		addItem(nRow, 7, sbQty);
		addItem(nRow, 8, bbPrice);
		addItem(nRow, 9, ssPrice);	
		addItem(nRow, 10, bbQty);	
		addItem(nRow, 11, ssQty);
	}
	else{
		...
		QString vol = QString::number(USTPMutexId::getActionNum(key));
		updateItemVolume(nSelRow, 3, vol);	
		updateItemPrice(nSelRow, 4, bsPrice);
		updateItemVolume(nSelRow, 5, bsQty);
		updateItemPrice(nSelRow, 6, sbPrice);
		updateItemVolume(nSelRow, 7, sbQty);
		updateItemPrice(nSelRow, 8, bbPrice);
		updateItemPrice(nSelRow, 9, ssPrice);
		updateItemVolume(nSelRow, 10, bbQty);	
		updateItemVolume(nSelRow, 11, ssQty);
	}
}
