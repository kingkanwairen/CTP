...
int USTPTradeApi::reqParkedOrderInsert(const int& reqId, QString& orderRef, const QString& brokerId, const QString& userId, const QString& investorId, const QString& instrumentId, const char& priceType, 
								 const char& timeCondition, const double& orderPrice, const int& volume, const char& direction, const char& offsetFlag, const char& hedgeFlag, const char& volumeCondition)
{
	if (mThis->m_pTradeApi != NULL){		
		QMutexLocker locker(&mThis->mOrderMutex);
		...
		req.ContingentCondition = THOST_FTDC_CC_Immediately;
		req.RequestID = reqId;
		int nResult = mThis->m_pTradeApi->ReqParkedOrderInsert(&req, reqId);
		return nResult;
	}
	return -1;
}

int USTPTradeApi::reqParkedOrderAction(const int& reqId, const QString& brokerId, const QString& userId, const QString& investorId, const QString& instrumentId, const QString& orderRef,
								const int& frontId, const int& sessionId)
{
	if (mThis->m_pTradeApi != NULL){
		QMutexLocker locker(&mThis->mOrderMutex);
		...
		req.SessionID = sessionId;
		req.ActionFlag = THOST_FTDC_AF_Delete;
		int nResult = mThis->m_pTradeApi->ReqParkedOrderAction(&req, reqId);
		return nResult;
	}
	return -1;
}

int USTPTradeApi::reqParkedOrderCancel(const int& reqId, const QString& brokerId, const QString& investorId, const QString& parkedOrderId)
{
	if (mThis->m_pTradeApi != NULL){
		QMutexLocker locker(&mThis->mOrderMutex);
		...
		strcpy(req.BrokerID, brokerId.toStdString().data());
		strcpy(req.InvestorID, investorId.toStdString().data());
		strcpy(req.ParkedOrderID, parkedOrderId.toStdString().data());
		int nResult = mThis->m_pTradeApi->ReqRemoveParkedOrder(&req, reqId);
		return nResult;
	}
	return -1;
}
