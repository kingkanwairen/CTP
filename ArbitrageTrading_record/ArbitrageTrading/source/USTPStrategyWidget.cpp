...
{	
	...
	for(int nIndex = 0; nIndex < orderList.size(); nIndex++){
		...
		doCreateNewOrder(cellItems.at(0), cellItems.at(1), cellItems.at(2), cellItems.at(3), cellItems.at(4),
			cellItems.at(22), cellItems.at(6).toDouble(), cellItems.at(5).toInt(), cellItems.at(7).toInt(), cellItems.at(8).toInt(), cellItems.at(9).toInt(),
			...
			cellItems.at(16).toInt(), cellItems.at(17).toInt(), cellItems.at(18).toInt(), cellItems.at(19).toInt(), cellItems.at(20).toInt(), cellItems.at(21).toInt());
	}
	return true;
}
...
void USTPStrategyWidget::createNewOrder(const int& row, const double& firstBidPrice, const double& firstAskPrice, const double& secBidPrice, const double& secAskPrice)
{
	...
	QCheckBox* defineOrderCheck = qobject_cast<QCheckBox *>(cellWidget(row, 17));
	bool isDefineOrder = defineOrderCheck->isChecked();

	QCheckBox* oppentPriceCheck = qobject_cast<QCheckBox *>(cellWidget(row, 18));
	bool isOppnentPrice = oppentPriceCheck->isChecked();

	QCheckBox* cancelFirstCheck = qobject_cast<QCheckBox *>(cellWidget(row, 19));
	bool isCancelFirst = cancelFirstCheck->isChecked();

	QCheckBox* cancelSecondCheck = qobject_cast<QCheckBox *>(cellWidget(row, 20));
	bool isCancelSecond = cancelSecondCheck->isChecked();

	QCheckBox* cycleCheck = qobject_cast<QCheckBox *>(cellWidget(row, 21));
	bool isCycle = cycleCheck->isChecked();

	QCheckBox* actionReferCheck = qobject_cast<QCheckBox *>(cellWidget(row, 22));
	bool isActionRefer = actionReferCheck->isChecked();

	QComboBox* hedgeCombox = qobject_cast<QComboBox *>(cellWidget(row, 23));
	...
	createStrategy(orderType, firstName, secName, price, oneOpenSlip, secOpenSlip, qty, direction, offstFlag, hedgeFlag, 
		cancelFirstTime, cancelSecTime, cyclePriceStall, actionReferTickNum, actionSuperSlipNum, isCancelFirst, isCancelSecond, isCycle, 
		isOppnentPrice, isDefineOrder, isActionRefer, firstBidPrice, firstAskPrice, secBidPrice, secAskPrice);
}
...
void USTPStrategyWidget::createStrategy(const int& orderType, const QString& firstIns, const QString& secIns, const double& orderPriceTick, const int& firstPriceSlipPoint, const int& secPriceSlipPoint, 
										...
										const int& actionSuperNum, bool isAutoFirstCancel, bool isAutoSecCancel, bool isCycle, bool isOppentPrice, bool isDefineOrder, bool isActionRefer, const double& firstBidPrice,
										const double& firstAskPrice, const double& secBidPrice, const double& secAskPrice)
{
	switch (orderType){
		case 0:
			{
		...
		case 4:
			{
				QString label = tr("[对价盯价套利]") + QString::number(USTPMutexId::getNewOrderIndex());
				USTPStrategyBase* pStrategy = new USTPOpponentStareArbitrage(label,  firstIns, secIns, orderPriceTick, firstPriceSlipPoint, secPriceSlipPoint, qty, bs, 
					offset, hedge, cancelFirstTime, cancelSecTime, cycleStall, actionReferNum, actionSuperNum, isAutoFirstCancel, isAutoSecCancel, isCycle, isOppentPrice,
					isDefineOrder, isActionRefer, firstBidPrice, firstAskPrice, secBidPrice, secAskPrice, mOrderWidget, mCancelWidget, this);
				mUserStrategys.insert(label, pStrategy);
			}
...
QString USTPStrategyWidget::getList()
{
	...
	for(int row = 0; row < nRowLen; row++){
		...
		QString szLine = firstName + QString(tr("|")) + secName + QString(tr("|")) + orderType + QString(tr("|")) + direction + QString(tr("|")) + 
			offstFlag + QString(tr("|")) + QString::number(qty) + QString(tr("|")) + QString::number(price, 'g', 3) + QString(tr("|")) + 
			QString::number(cancelFirstTime) + QString(tr("|")) + QString::number(cancelSecTime) + QString(tr("|")) + QString::number(oneOpenSlip) + QString(tr("|")) + 
			QString::number(secOpenSlip) +  QString(tr("|")) +  QString::number(referTickName) + QString(tr("|")) + QString::number(superPriceNum) + QString(tr("|")) + 
			QString::number(cyclePriceStall) + QString(tr("|")) + QString::number(actionReferNum) + QString(tr("|")) + QString::number(actionSuperNum) + QString(tr("|")) + 
			QString::number(isDefineOrder ? 1 : 0) + QString(tr("|")) + QString::number(isOppnentPrice ? 1 : 0) + QString(tr("|")) + QString::number(isCancelFirst ? 1 : 0) +
			QString(tr("|")) + QString::number(isCancelSecond ? 1 : 0) + QString(tr("|")) + QString::number(isCycle ? 1 : 0) + QString(tr("|")) + QString::number(isActonRefer ? 1 : 0) + 
			QString(tr("|")) + hedgeFlag;
		if(row == 0)
			szSaveData = szLine;
		else
...
{	
	...
	if(isEnbed){
		doCreateNewOrder(firstIns, secIns, orderStyle, direction, offsetFlag, hedgeFlag, priceTick, orderQty, 0, CANCEL_DELAY_TIME, SUPER_PRICE_STALL,
			SUPER_PRICE_STALL, REFER_PRICE_NUM, 0, 0, REFER_PRICE_NUM, 0, false, false, false, true, false, true);
	}else{
		...
		createStrategy(orderType, firstIns, secIns, priceTick, SUPER_PRICE_STALL, SUPER_PRICE_STALL, orderQty, bs, offstFlag, hFlag, 
			0, CANCEL_DELAY_TIME, 0, REFER_PRICE_NUM, 0, false, true, false, false, false, true, firstMarketBidPrice, firstMarketAskPrice, 
