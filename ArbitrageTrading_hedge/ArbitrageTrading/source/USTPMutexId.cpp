...
bool USTPMutexId::getDecodeIns(const QString& combIns, QString& firstIns, QString& secIns)
{
	if(mThis->mInsExhs.find(combIns) != mThis->mInsExhs.end()){
		int pos = combIns.indexOf(" ");
		QString temIns = combIns.mid(pos + 1);
		int insPos = temIns.indexOf("&");
		firstIns = temIns.mid(0, insPos);
		secIns = temIns.mid(insPos + 1);
		return true;
	}
	return false;
}
