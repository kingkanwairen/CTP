...
USTPStrategyBase::USTPStrategyBase(const QString& orderLabel, const QString& firstIns, const QString& secIns, const double& orderPriceTick, const int& qty, const char& bs,  const char& offset,
								   const char& hedge, const int& cancelFirstTime, const int& cancelSecTime, const int& cycleStall, const int& mdQty, const int& firstSlipPoint, const int& secSlipPoint,
								   bool isAutoFirstCancel, bool isAutoSecCancel, bool isCycle)
{	
	...
	mFirstInsPriceTick = USTPMutexId::getInsPriceTick(firstIns);
	mSecInsPriceTick = USTPMutexId::getInsPriceTick(secIns);
...
USTPStarePriceArbitrage::USTPStarePriceArbitrage(const QString& orderLabel, const QString& firstIns, const QString& secIns, const double& orderPriceTick, const int& secPriceSlipPoint, const int& qty, const char& bs,  const char& offset, 
												 const char& hedge, const int& cancelFirstTime, const int& cancelSecTime, const int& cycleStall, const int& mdQty, bool isAutoFirstCancel, bool isAutoSecCancel, bool isCycle, const double& firstMarketBidPrice,
												 ...
												 :USTPStrategyBase(orderLabel, firstIns, secIns, orderPriceTick, qty, bs, offset, hedge, cancelFirstTime, cancelSecTime, cycleStall, mdQty, 0, secPriceSlipPoint, isAutoFirstCancel, isAutoSecCancel, isCycle)
{		
	...
	mSecondSlipPrice = secPriceSlipPoint * mSecInsPriceTick;
	mBrokerId = USTPCtpLoader::getBrokerId();
	mUserId = USTPMutexId::getUserId();
	mInvestorId = USTPMutexId::getInvestorId();
	double maxValue = mFirstInsPriceTick > mSecInsPriceTick ? mFirstInsPriceTick : mSecInsPriceTick;
	mLimitSpread = USTPMutexId::getLimitSpread(mInsComplex) * maxValue;
...
void USTPStarePriceArbitrage::initConnect(USTPStrategyWidget* pStrategyWidget, USTPOrderWidget* pOrderWidget, USTPCancelWidget* pCancelWidget)
{
	...
	connect(this, SIGNAL(onOrderFinished(const QString&, const QString&, const QString&, const double&, const int&, const int&, const int&, const char&,  const char&, const char&, const int&, const int&, const int&, const int&, const int&, 
		const int&, bool, bool, bool, bool, bool, bool, bool, const int&, const double&, const double&, const double&, const double&)), pStrategyWidget, SLOT(doOrderFinished(const QString&, const QString&, const QString&, const double&, const int&, 
		const int&, const int&, const char&,  const char&, const char&, const int&, const int&, const int&, const int&, const int&, const int&, bool, bool, bool, bool, bool, bool, bool, const int&, const double&, const double&, const double&,
		const double&)), Qt::QueuedConnection);

	connect(this, SIGNAL(onOrderFinished(const QString&, const QString&, const QString&, const double&, const int&, const int&, const int&, const char&,  const char&, const char&, const int&, const int&, const int&, const int&, const int&, 
		const int&, bool, bool, bool, bool, bool, bool, bool, const int&, const double&, const double&, const double&, const double&)), pCancelWidget, SLOT(doOrderFinished(const QString&, const QString&, const QString&, const double&, const int&, 
		const int&, const int&, const char&,  const char&, const char&, const int&, const int&, const int&, const int&, const int&, const int&, bool, bool, bool, bool, bool, bool, bool, const int&, const double&, const double&, const double&, 
		const double&)), Qt::QueuedConnection);
}
...
{	
	...
	if(isFirstIns){
		...
		double firstTemValue = adjustPrice / mFirstInsPriceTick;
		if(direction == '0')
			adjustPrice = floor(firstTemValue) * mFirstInsPriceTick;
		else
			adjustPrice = ceil(firstTemValue) * mFirstInsPriceTick;
		mFirstOrderPrice = adjustPrice;
		mFirstOrderBasePrice = adjustPrice - mOrderPrice;
		...
		double secTemValue = adjustPrice / mSecInsPriceTick;
		if(direction == '0')
			adjustPrice = floor(secTemValue) * mFirstInsPriceTick;
		else
			adjustPrice = ceil(secTemValue) * mFirstInsPriceTick;
		USTPTradeApi::reqOrderInsert(reqId, orderRef, mBrokerId, mUserId, mInvestorId, instrument, priceType, timeCondition, adjustPrice, qty, direction, mOffsetFlag, mHedgeFlag, THOST_FTDC_VC_AV);
	...
	if(mFirstOrderRef == orderRef){
		...
		if (THOST_FTDC_OST_Canceled == orderStatus && mIsDeleted){
			emit onOrderFinished(mOrderLabel, mFirstIns, mSecIns, mOrderPrice, mFirstSlipPoint, mSecSlipPoint, mOrderQty, mBS, mOffsetFlag, mHedgeFlag, mFirstCancelTime,
				mSecondCancelTime, mCycleStall, mActionReferNum, mActionSuperNum, mMinMdQty, mIsAutoFirstCancel, mIsAutoSecondCancel, mIsCycle, false, false, false, mIsActionReferTick,
											 ...
											 const QString& localId, const QString& orderRef, const QString& tradeTime)
{
	...
	if(mFirstTradeQty >= mOrderQty && mSecondTradeQty >= mOrderQty){
		emit onOrderFinished(mOrderLabel, mFirstIns, mSecIns, mOrderPrice, mFirstSlipPoint, mSecSlipPoint, mOrderQty, mBS, mOffsetFlag, mHedgeFlag, mFirstCancelTime,
			mSecondCancelTime, mCycleStall, mActionReferNum, mActionSuperNum, mMinMdQty, mIsAutoFirstCancel, mIsAutoSecondCancel, mIsCycle, false, false, true, mIsActionReferTick,
			mOrderType, 0.0, 0.0, mSecondBidMarketPrice, mSecondAskMarketPrice);
	}
}
...
{	
	if (mIsCanMarket && (EXH_CZCE == mExh || EXH_CFFEX == mExh)){
		if (THOST_FTDC_D_Buy == mBS)
...
USTPConditionArbitrage::USTPConditionArbitrage(const QString& orderLabel, const QString& firstIns, const QString& secIns, const double& orderPriceTick, const int& firstPriceSlipPoint, const int& secPriceSlipPoint, 
											   const int& qty, const char& bs,  const char& offset, const char& hedge, const int& cancelFirstTime, const int& cancelSecTime, const int& cycleStall, const int& mdQty, const int& actionReferNum, 
											   ...
											   USTPStrategyWidget* pStrategyWidget) :USTPStrategyBase(orderLabel, firstIns, secIns, orderPriceTick, qty, bs, offset, hedge, cancelFirstTime, cancelSecTime, cycleStall, mdQty, firstPriceSlipPoint,
...
{	
	...
	mFirstInsStatus = USTP_FTDC_OS_ORDER_NO_ORDER;
	mSecInsStatus = USTP_FTDC_OS_ORDER_NO_ORDER;
	mFirstSlipPrice = firstPriceSlipPoint * mFirstInsPriceTick;
	mSecondSlipPrice = secPriceSlipPoint * mSecInsPriceTick;
	mActionSuperSlipPrice = actionSuperNum * mFirstInsPriceTick;
	mBrokerId = USTPCtpLoader::getBrokerId();
	mUserId = USTPMutexId::getUserId();
	mInvestorId = USTPMutexId::getInvestorId();
...
{	
	...
	if(mFirstIns == instrumentId){
		...
		mSecondBidMarketPrice = bidPrice;
		mSecondAskMarketPrice = askPrice;
		mSecBidMarketVolume = bidVolume;
		mSecAskMarketVolume = askVolume;
	}
...
void USTPConditionArbitrage::opponentPriceOrder(const QString& instrument, const double& basePrice)
{
	if(((USTP_FTDC_OS_ORDER_NO_ORDER == mFirstInsStatus) || (THOST_FTDC_OST_Canceled == mFirstInsStatus))){//第一腿没有下单或者已撤单
		if(THOST_FTDC_D_Buy == mBS && mFirstAskMarketPrice < basePrice - VALUE_ACCURACY){	//买委托满足下单条件
			...
			mSecondOrderBasePrice = mSecondMarketBasePrice;
			switchFirstInsOrder(THOST_FTDC_TC_GFD);
		}else if(THOST_FTDC_D_Buy == mBS && mFirstAskMarketPrice > basePrice - VALUE_ACCURACY && mFirstAskMarketPrice < basePrice + VALUE_ACCURACY){	//买委托满足下单条件
			if((mIsDefineOrder && mSecBidMarketVolume >= mMinMdQty) || !mIsDefineOrder){
				mFirstOrderPrice = mFirstAskMarketPrice;
				mSecondOrderBasePrice = mSecondMarketBasePrice;
				switchFirstInsOrder(THOST_FTDC_TC_GFD);
			}

		}else if(THOST_FTDC_D_Sell == mBS && mFirstBidMarketPrice > basePrice - VALUE_ACCURACY && mFirstBidMarketPrice < basePrice + VALUE_ACCURACY){//卖委托满足下单条件
			if((mIsDefineOrder && mSecAskMarketVolume >= mMinMdQty) || !mIsDefineOrder){
				mFirstOrderPrice = mFirstBidMarketPrice;
				mSecondOrderBasePrice = mSecondMarketBasePrice;
				switchFirstInsOrder(THOST_FTDC_TC_GFD);
			}
		...
		if((mSecIns == instrument) && ((THOST_FTDC_D_Buy == mBS && (mFirstOrderPrice - VALUE_ACCURACY> actionBidBasePrice)) || (THOST_FTDC_D_Sell == mBS && (mFirstOrderPrice + VALUE_ACCURACY< actionAskBasePrice)))){
			...
			cancelFirstIns();
		}else if((THOST_FTDC_D_Buy == mBS && (mFirstAskMarketPrice > mFirstOrderPrice + VALUE_ACCURACY)) || (THOST_FTDC_D_Sell == mBS && (mFirstBidMarketPrice < mFirstOrderPrice - VALUE_ACCURACY))){
			cancelFirstIns();
		}else if(mIsDefineOrder && ((THOST_FTDC_D_Buy == mBS && (mFirstAskMarketPrice > mFirstOrderPrice - VALUE_ACCURACY) && (mFirstAskMarketPrice < mFirstOrderPrice + VALUE_ACCURACY) && mSecBidMarketVolume < mMinMdQty) || 
			(THOST_FTDC_D_Sell == mBS && (mFirstBidMarketPrice < mFirstOrderPrice - VALUE_ACCURACY) && (mFirstBidMarketPrice > mFirstOrderPrice + VALUE_ACCURACY) && mSecAskMarketVolume < mMinMdQty))){
			cancelFirstIns();
		}
	}
}
...
void USTPConditionArbitrage::noOpponentPriceOrder(const QString& instrument, const double& basePrice)
{
	if(((USTP_FTDC_OS_ORDER_NO_ORDER == mFirstInsStatus) || (THOST_FTDC_OST_Canceled == mFirstInsStatus))){//第一腿没有下单或者已撤单
		...
		if(THOST_FTDC_D_Buy == mBS && mFirstMarketBasePrice < basePrice - VALUE_ACCURACY){	//条件单买委托满足下单条件
			...
			switchFirstInsOrder(THOST_FTDC_TC_GFD);
#ifdef _DEBUG
			QString data = mOrderLabel + QString(tr("  [NoOpponentPriceOrder] #1 ")) + tr("  FirstMarketBasePrice: ") +  QString::number(mFirstMarketBasePrice) + tr("  BasePrice: ") + QString::number(basePrice) +
				tr("  FirstOrderPrice: ") +  QString::number(mFirstOrderPrice) + tr(" mFirstSlipPrice: ") +  QString::number(mFirstSlipPrice);
			USTPLogger::saveData(data);
#endif
			...
			switchFirstInsOrder(THOST_FTDC_TC_GFD);
#ifdef _DEBUG
			QString data = mOrderLabel + QString(tr("  [NoOpponentPriceOrder] #2 ")) + tr("  FirstMarketBasePrice: ") +  QString::number(mFirstMarketBasePrice) + tr("  BasePrice: ") + QString::number(basePrice) +
				tr("  FirstOrderPrice: ") +  QString::number(mFirstOrderPrice) + tr(" FirstSlipPrice: ") +  QString::number(mFirstSlipPrice);
			USTPLogger::saveData(data);
#endif

		}else if(THOST_FTDC_D_Buy == mBS && mFirstMarketBasePrice > basePrice - VALUE_ACCURACY && mFirstMarketBasePrice < basePrice + VALUE_ACCURACY){	//买委托满足下单条件
			if((mIsDefineOrder && mSecBidMarketVolume >= mMinMdQty) || !mIsDefineOrder){
				mFirstOrderPrice = mFirstMarketBasePrice + mFirstSlipPrice;
				if(mFirstOrderPrice > basePrice)
					mFirstOrderPrice = basePrice;
				...
				switchFirstInsOrder(THOST_FTDC_TC_GFD);
#ifdef _DEBUG
				QString data = mOrderLabel + QString(tr("  [NoOpponentPriceOrder] #3 ")) + tr("  FirstMarketBasePrice: ") +  QString::number(mFirstMarketBasePrice) + tr("  BasePrice: ") + QString::number(basePrice) +
					tr("  FirstOrderPrice: ") +  QString::number(mFirstOrderPrice) + tr(" FirstSlipPrice: ") +  QString::number(mFirstSlipPrice) + tr(" SecBidMarketVolume: ") +  QString::number(mSecBidMarketVolume);
				USTPLogger::saveData(data);
#endif
			}
		}else if(THOST_FTDC_D_Sell == mBS && mFirstMarketBasePrice > basePrice - VALUE_ACCURACY && mFirstMarketBasePrice < basePrice + VALUE_ACCURACY){//卖委托满足下单条件
			if((mIsDefineOrder && mSecAskMarketVolume >= mMinMdQty) || !mIsDefineOrder){
				mFirstOrderPrice = mFirstMarketBasePrice - mFirstSlipPrice;
				if(mFirstOrderPrice < basePrice)
					mFirstOrderPrice = basePrice;
				...
				switchFirstInsOrder(THOST_FTDC_TC_GFD);
#ifdef _DEBUG
				QString data = mOrderLabel + QString(tr("  [NoOpponentPriceOrder] #4 ")) + tr("  FirstMarketBasePrice: ") +  QString::number(mFirstMarketBasePrice) + tr("  BasePrice: ") + QString::number(basePrice) +
					tr("  FirstOrderPrice: ") +  QString::number(mFirstOrderPrice) + tr(" FirstSlipPrice: ") +  QString::number(mFirstSlipPrice) + tr(" SecAskMarketVolume: ") +  QString::number(mSecAskMarketVolume);
				USTPLogger::saveData(data);
#endif
			}
		}else{
		...
		if((mSecIns == instrument) && ((THOST_FTDC_D_Buy == mBS && (mFirstOrderPrice - VALUE_ACCURACY > actionBidBasePrice)) || (THOST_FTDC_D_Sell == mBS && (mFirstOrderPrice + VALUE_ACCURACY < actionAskBasePrice)))){
			cancelFirstIns();
#ifdef _DEBUG
			QString data = mOrderLabel + QString(tr("  [CancelOrder] #1 "));
			USTPLogger::saveData(data);
#endif
			...
			cancelFirstIns();
#ifdef _DEBUG
			QString data = mOrderLabel + QString(tr("  [CancelOrder] #2 "));
			USTPLogger::saveData(data);
#endif
			...
			(THOST_FTDC_D_Sell == mBS && (mIsActionReferTick && (mCurrentReferIndex >= mActionReferNum)) && (mFirstMarketBasePrice > basePrice))){
				cancelFirstIns();
#ifdef _DEBUG
				QString data = mOrderLabel + QString(tr("  [CancelOrder] #3 "));
				USTPLogger::saveData(data);
#endif
		}else if(mIsDefineOrder && (((mFirstOrderPrice > basePrice - VALUE_ACCURACY) && (mFirstOrderPrice < basePrice + VALUE_ACCURACY)) && ((THOST_FTDC_D_Buy == mBS && (mSecBidMarketVolume < mMinMdQty)) || 
			(THOST_FTDC_D_Sell == mBS && (mSecAskMarketVolume < mMinMdQty))))){
			cancelFirstIns();
#ifdef _DEBUG
			QString data = mOrderLabel + QString(tr("  [CancelOrder] #4 ")) + tr("  SecBidMarketVolume: ") +  QString::number(mSecBidMarketVolume) + tr("  MinMdQty: ") + QString::number(mMinMdQty) +
				tr("  SecAskMarketVolume : ") +  QString::number(mSecAskMarketVolume);
			USTPLogger::saveData(data);
#endif
		}else if(((mFirstMarketBasePrice > mFirstOrderPrice - VALUE_ACCURACY) && (mFirstMarketBasePrice < mFirstOrderPrice + VALUE_ACCURACY)) && ((THOST_FTDC_D_Buy == mBS && (mFirstOrderPrice < basePrice - VALUE_ACCURACY)
			&& (mFirstRemainQty < mFirstBidMarketVolume)) || (THOST_FTDC_D_Sell == mBS && (mFirstOrderPrice > basePrice + VALUE_ACCURACY) && (mFirstRemainQty < mFirstAskMarketVolume)))){
				if (THOST_FTDC_D_Buy == mBS)
					mFirstMarketBasePrice += mFirstInsPriceTick;
				else
					mFirstMarketBasePrice -= mFirstInsPriceTick;	
				cancelFirstIns();
#ifdef _DEBUG
				QString data = mOrderLabel + QString(tr("  [CancelOrder] #5 "));
				USTPLogger::saveData(data);
#endif
		}
	...
	if(((USTP_FTDC_OS_ORDER_NO_ORDER == mFirstInsStatus) || (THOST_FTDC_OST_Canceled == mFirstInsStatus))){//第一腿没有下单或者已撤单
		if(mIsOppnentPrice){
			if(THOST_FTDC_D_Buy == mBS && mFirstAskMarketPrice < basePrice - VALUE_ACCURACY){	//买委托满足下单条件
				...
				switchFirstInsOrder(THOST_FTDC_TC_GFD);
#ifdef _DEBUG
				QString data = mOrderLabel + QString(tr("  [Opponent-DefineOrder] #1 ")) + tr("  FirstAskMarketPrice: ") +  QString::number(mFirstAskMarketPrice) + tr("  BasePrice: ") + QString::number(basePrice);
				USTPLogger::saveData(data);
#endif
				...
				switchFirstInsOrder(THOST_FTDC_TC_GFD);
#ifdef _DEBUG
				QString data = mOrderLabel + QString(tr("  [Opponent-DefineOrder] #2 ")) + tr("  FirstBidMarketPrice: ") +  QString::number(mFirstBidMarketPrice) + tr("  BasePrice: ") + QString::number(basePrice);
				USTPLogger::saveData(data);
#endif
			}else if(THOST_FTDC_D_Buy == mBS && mFirstAskMarketPrice > basePrice - VALUE_ACCURACY && mFirstAskMarketPrice < basePrice + VALUE_ACCURACY){	//买委托满足下单条件
				if((mIsDefineOrder && mSecBidMarketVolume >= mMinMdQty) || !mIsDefineOrder){
...
#ifdef _DEBUG
				QString data = mOrderLabel + QString(tr("  [Opponent-DefineOrder] #3 ")) + tr("  FirstAskMarketPrice: ") +  QString::number(mFirstAskMarketPrice) + tr("  BasePrice: ") + QString::number(basePrice) +
					 tr("  SecBidMarketVolume: ") +  QString::number(mSecBidMarketVolume) + tr("  MinMdQty: ") +  QString::number(mMinMdQty) ;
				USTPLogger::saveData(data);
#endif
...
#ifdef _DEBUG
				QString data = mOrderLabel + QString(tr("  [Opponent-DefineOrder] #4 ")) + tr("  FirstBidMarketPrice: ") +  QString::number(mFirstBidMarketPrice) + tr("  BasePrice: ") + QString::number(basePrice) +
					tr("  SecAskMarketVolume: ") +  QString::number(mSecAskMarketVolume) + tr("  MinMdQty: ") +  QString::number(mMinMdQty) ;
				USTPLogger::saveData(data);
#endif
			}else{
				if(THOST_FTDC_OST_Canceled == mFirstInsStatus){		
			...
			if(THOST_FTDC_D_Buy == mBS && mFirstMarketBasePrice < basePrice - VALUE_ACCURACY){	//条件单买委托满足下单条件
				...
				switchFirstInsOrder(THOST_FTDC_TC_GFD);
#ifdef _DEBUG
				QString data = mOrderLabel + QString(tr("  [Condition-DefineOrder] #1 ")) + tr("  FirstMarketBasePrice: ") +  QString::number(mFirstMarketBasePrice) + tr("  BasePrice: ") + QString::number(basePrice);
				USTPLogger::saveData(data);
#endif
				...
				switchFirstInsOrder(THOST_FTDC_TC_GFD);
#ifdef _DEBUG
				QString data = mOrderLabel + QString(tr("  [Condition-DefineOrder] #2 ")) + tr("  FirstMarketBasePrice: ") +  QString::number(mFirstMarketBasePrice) + tr("  BasePrice: ") + QString::number(basePrice);
				USTPLogger::saveData(data);
#endif
...
#ifdef _DEBUG
				QString data = mOrderLabel + QString(tr("  [Condition-DefineOrder] #3 ")) + tr("  FirstMarketBasePrice: ") +  QString::number(mFirstMarketBasePrice) + tr("  BasePrice: ") + QString::number(basePrice)
					+ tr("  SecBidMarketVolume: ") +  QString::number(mSecBidMarketVolume) + tr("  MinMdQty: ") +  QString::number(mMinMdQty);
				USTPLogger::saveData(data);
#endif
...
#ifdef _DEBUG
				QString data = mOrderLabel + QString(tr("  [Condition-DefineOrder] #4 ")) + tr("  FirstMarketBasePrice: ") +  QString::number(mFirstMarketBasePrice) + tr("  BasePrice: ") + QString::number(basePrice)
					+ tr("  SecAskMarketVolume: ") +  QString::number(mSecAskMarketVolume) + tr("  MinMdQty: ") +  QString::number(mMinMdQty);
				USTPLogger::saveData(data);
#endif
...
{	
	...
	if(mFirstOrderRef == orderRef){
		...
		if (THOST_FTDC_OST_Canceled == orderStatus && mIsDeleted){
			emit onOrderFinished(mOrderLabel, mFirstIns, mSecIns, mOrderPrice, mFirstSlipPoint, mSecSlipPoint, mOrderQty, mBS, mOffsetFlag, mHedgeFlag, mFirstCancelTime,
				mSecondCancelTime, mCycleStall, mActionReferNum, mActionSuperNum, mMinMdQty, mIsAutoFirstCancel, mIsAutoSecondCancel, mIsCycle, mIsOppnentPrice, mIsDefineOrder, false,
...
void USTPOpponentArbitrage::noOpponentPriceOrder(const QString& instrument, const double& basePrice)
{
	if(((USTP_FTDC_OS_ORDER_NO_ORDER == mFirstInsStatus) || (THOST_FTDC_OST_Canceled == mFirstInsStatus))){//第一腿没有下单或者已撤单
		...
		USTPLogger::saveData(data);
#endif
		if((mSecIns == instrument) && ((THOST_FTDC_D_Buy == mBS && (mFirstOrderPrice > actionBidBasePrice + VALUE_ACCURACY)) || (THOST_FTDC_D_Sell == mBS && (mFirstOrderPrice < actionAskBasePrice - VALUE_ACCURACY)))){
			cancelFirstIns();
#ifdef _DEBUG
			...
			USTPLogger::saveData(data);
#endif
		}else if((THOST_FTDC_D_Buy == mBS && (mFirstMarketBasePrice - VALUE_ACCURACY > mFirstOrderPrice)&& (mFirstMarketBasePrice < basePrice - VALUE_ACCURACY)) || 
			(THOST_FTDC_D_Sell == mBS && (mFirstMarketBasePrice  + VALUE_ACCURACY < mFirstOrderPrice)&& (mFirstMarketBasePrice > basePrice + VALUE_ACCURACY))){
				cancelFirstIns();
#ifdef _DEBUG
...
USTPConditionStareArbitrage::USTPConditionStareArbitrage(const QString& orderLabel, const QString& firstIns, const QString& secIns, const double& orderPriceTick, const int& firstPriceSlipPoint, const int& secPriceSlipPoint, 
														 const int& qty, const char& bs,  const char& offset, const char& hedge, const int& cancelFirstTime, const int& cancelSecTime, const int& cycleStall, const int& mdQty, const int& actionReferNum, const int& actionSuperNum,
														 ...
														 :USTPStrategyBase(orderLabel, firstIns, secIns, orderPriceTick, qty, bs, offset, hedge, cancelFirstTime, cancelSecTime, cycleStall, mdQty, firstPriceSlipPoint,
														 secPriceSlipPoint, isAutoFirstCancel, isAutoSecCancel, isCycle)
{		
	moveToThread(&mStrategyThread);
	mStrategyThread.start();
...
{	
	...
	double fSecBasePrice = mSecondMarketBasePrice + mOrderPrice;
#ifdef _DEBUG
	QString data = tr("[") + mOrderLabel + tr("-Depth]   FirstMarketBasePrice: ") + QString::number(mFirstMarketBasePrice) + tr("  FirstBidMarketPrice: ") +
		QString::number(mFirstBidMarketPrice)  + tr("  FirstAskMarketPrice: ") + QString::number(mFirstAskMarketPrice) + tr("  BasePrice: ") + 
		QString::number(fSecBasePrice) + tr("  OrderPrice: ") + QString::number(mFirstOrderPrice);
	USTPLogger::saveData(data);
#endif
...
void USTPConditionStareArbitrage::conditionStarePriceOrder(const QString& instrument, const double& basePrice)
{
	if(((USTP_FTDC_OS_ORDER_NO_ORDER == mFirstInsStatus) || (THOST_FTDC_OST_Canceled == mFirstInsStatus))){
		...
		if((mSecIns == instrument) && ((THOST_FTDC_D_Buy == mBS && (mFirstOrderPrice > actionBidBasePrice + VALUE_ACCURACY)) || (THOST_FTDC_D_Sell == mBS && (mFirstOrderPrice < actionAskBasePrice - VALUE_ACCURACY)))){
			...
			USTPLogger::saveData(data);
#endif
		}else if((THOST_FTDC_D_Buy == mBS && (mFirstMarketBasePrice > mFirstOrderPrice + VALUE_ACCURACY) && (mFirstMarketBasePrice < basePrice - VALUE_ACCURACY)) || 
			(THOST_FTDC_D_Sell == mBS && (mFirstMarketBasePrice < mFirstOrderPrice  - VALUE_ACCURACY) && (mFirstMarketBasePrice > basePrice + VALUE_ACCURACY))){
				cancelFirstIns();
#ifdef _DEBUG
...
{	
	...
	int nIsSecIns = isFirstIns ? 0 : 1;
	QString data = mOrderLabel + QString(tr("  [")) + insLabel + QString(tr("-OrderInsert]   Instrument: ")) + instrument +  QString(tr("  RequestId: ")) + szReqId + QString(tr("  OrderRef: ")) + orderRef + 
		QString(tr("  UserId: ")) + mUserId + QString(tr("  PriceType: ")) + QString(priceType) + QString(tr("  AdjustPrice: ")) + QString::number(adjustPrice) + QString(tr("  OrderPrice: ")) + 
		QString::number(orderPrice) +  QString(tr("  FirstOrderPrice: ")) + QString::number(mFirstOrderPrice) + QString(tr("  OrderVolume: ")) + QString::number(qty) + QString(tr("  Direction: ")) +
		QString(direction) + QString(tr("  SecAutoCancel: ")) + QString::number(nIsSecCancel) + QString(tr("  IsSecIns: ")) + QString::number(nIsSecIns);
	USTPLogger::saveData(data);
#endif	
}
...
USTNewConditionArbitrage::USTNewConditionArbitrage(const QString& orderLabel, const QString& firstIns, const QString& secIns, const double& orderPriceTick, const int& firstPriceSlipPoint, const int& secPriceSlipPoint, 
											 const int& qty, const char& bs,  const char& offset, const char& hedge, const int& cancelFirstTime, const int& cancelSecTime, const int& cycleStall, const int& mdQty, const int& actionReferNum, 
											 const int& actionSuperNum, bool isAutoFirstCancel, bool isAutoSecCancel, bool isCycle, bool isOppentPrice, bool isDefineOrder, bool isReferTick, const double& firstMarketBidPrice,
											 const double& firstMarketAskPrice, const double& secMarketBidPrice, const double& secMarketAskPrice, USTPOrderWidget* pOrderWidget, USTPCancelWidget* pCancelWidget,
											 USTPStrategyWidget* pStrategyWidget) :USTPStrategyBase(orderLabel, firstIns, secIns, orderPriceTick, qty, bs, offset, hedge, cancelFirstTime, cancelSecTime, cycleStall, mdQty, firstPriceSlipPoint,
...
USTNewConditionArbitrage::~USTNewConditionArbitrage()
{
	mStrategyThread.quit();
	mStrategyThread.wait();
}

void USTNewConditionArbitrage::initConnect(USTPStrategyWidget* pStrategyWidget, USTPOrderWidget* pOrderWidget, USTPCancelWidget* pCancelWidget)
{
	connect(USTPCtpLoader::getMdSpi(), SIGNAL(onUSTPRtnDepthMarketData(const QString&, const double&, const double&, 
		const double&,const double&, const int&, const double&, const int&, const double&, const double&, const int&)), 
		this, SLOT(doUSTPRtnDepthMarketData(const QString&, const double&, const double&, 
...
void USTNewConditionArbitrage::updateInitShow()
{	
	if(mIsDeleted)	//如果手工撤单，则不更新
		return;
	mFirstInsStatus = USTP_FTDC_OS_ORDER_NO_ORDER;
...
void USTNewConditionArbitrage::doUSTPRtnDepthMarketData(const QString& instrumentId, const double& preSettlementPrice, const double& openPrice, const double& lastPrice,
													 const double& bidPrice, const int& bidVolume, const double& askPrice, const int& askVolume, const double& highestPrice, 
													 const double& lowestPrice, const int& volume)
{	
	if(mIsDefineOrder && mReferenceIns == instrumentId)
...
void USTNewConditionArbitrage::noOpponentPriceOrder(const QString& instrument, const double& basePrice)
{
	if(((USTP_FTDC_OS_ORDER_NO_ORDER == mFirstInsStatus) || (THOST_FTDC_OST_Canceled == mFirstInsStatus))){//第一腿没有下单或者已撤单
...
void USTNewConditionArbitrage::defineTimeOrder(const double& basePrice)
{
	if(((USTP_FTDC_OS_ORDER_NO_ORDER == mFirstInsStatus) || (THOST_FTDC_OST_Canceled == mFirstInsStatus))){//第一腿没有下单或者已撤单
		if(THOST_FTDC_D_Buy == mBS && mFirstMarketBasePrice < (basePrice - VALUE_ACCURACY)){	//条件单买委托满足下单条件
			mFirstOrderPrice = mFirstMarketBasePrice + mFirstSlipPrice;
...
void USTNewConditionArbitrage::switchFirstInsOrder(const char& tCondition)
{
	if(USTP_FTDC_OS_ORDER_NO_ORDER == mFirstInsStatus){
		if(mIsDeleted)
			return;
...
void USTNewConditionArbitrage::submitOrder(const QString& insLabel, const QString& instrument, const double& orderPrice, const char& direction, const int& qty, const char& priceType, const char& timeCondition, bool isFirstIns)
{	
	if(isFirstIns && mIsDeleted)	//撤掉报单，合约一禁止下新单
		return;
	orderInsert(USTPMutexId::getMutexId(), insLabel, instrument, orderPrice, direction, qty, priceType, timeCondition, isFirstIns);	
...
void USTNewConditionArbitrage::orderInsert(const int& reqId, const QString& insLabel, const QString& instrument, const double& orderPrice, const char& direction, const int& qty, const char& priceType, const char& timeCondition, bool isFirstIns)
{	
	if(USTPMutexId::getActionNum(mInsComplex) > MAX_CANCEL_NUM)
		return;
	double adjustPrice = (priceType == THOST_FTDC_OPT_LimitPrice) ? orderPrice : 0.0;
...
void USTNewConditionArbitrage::doUSTPRtnOrder(const QString& localId, const QString& orderRef, const QString& instrumentId, const char& direction, const double& orderPrice, const int& orderVolume,
										   const int& remainVolume, const int& tradeVolume, const char& offsetFlag, const char& priceType, const char& hedgeFlag, const char& orderStatus,
										   const QString& brokerId, const QString& exchangeId, const QString& investorId, const QString& orderSysId, const QString& statusMsg, const char& timeCondition, const int& reqId)
{	
	if(mReqMap.find(orderRef) == mReqMap.end())
...
void USTNewConditionArbitrage::doUSTPRtnTrade(const QString& tradeId, const QString& instrumentId, const char& direction, const int& tradeVolume, const double& tradePrice,
										   const char& offsetFlag, const char& hedgeFlag, const QString& brokerId, const QString& exchangeId, const QString& investorId, const QString& orderSysId,
										   const QString& localId, const QString& orderRef, const QString& tradeTime)
{
	if(mFirstOrderRef == orderRef){
...
void USTNewConditionArbitrage::orderSecondIns(bool isInit, const int& qty, const double& bidPrice, const double& askPrice)
{	
	if (mIsCanMarket && (EXH_CZCE == mExh || EXH_CFFEX == mExh)){
		if (THOST_FTDC_D_Buy == mBS)
			submitOrder(SECOND_INSTRUMENT, mSecIns, 0.0, THOST_FTDC_D_Sell, qty, THOST_FTDC_OPT_AnyPrice, THOST_FTDC_TC_IOC, false);
...
void USTNewConditionArbitrage::doUSTPErrRtnOrderInsert(const QString& userId, const QString& brokerId, const char& direction, const QString& gtdDate, const char& hedgeFlag,
													const QString& instrumentId, const QString& investorId, const char& offsetFlag, const char& priceType, const char& timeCondition,
													const QString& orderRef, const double& orderPrice, const int& volume, const int& errorId, const QString& errorMsg, const int& reqId)
{	
	if(mReqMap.find(orderRef) == mReqMap.end())
...
void USTNewConditionArbitrage::doUSTPErrRtnOrderAction(const char& actionFlag, const QString& brokerId, const QString& exchangeId, const QString& investorId,
													const QString& orderSysId, const QString& userActionLocalId, const QString& orderActionRef, const double& orderPrice, 
													const int& volumeChange, const int& errorId, const QString& errorMsg, const int& reqId)
{
#ifdef _DEBUG
...
void USTNewConditionArbitrage::doAutoCancelFirstIns()
{	
	if(isInMarket(mFirstInsStatus))
		cancelFirstIns();
}

void USTNewConditionArbitrage::cancelFirstIns()
{
	mFirstInsStatus = USTP_FTDC_OS_CANCEL_SUBMIT;
	submitAction(FIRST_INSTRUMENT, mFirstOrderRef, mFirstIns);
}

void USTNewConditionArbitrage::doAutoCancelSecIns()
{	
	if(mSecActionList.isEmpty()){
#ifdef _DEBUG
		QString data = mOrderLabel + QString(tr("  [WARN]  The list is empty when Auto cancel second instrument."));
...
void USTNewConditionArbitrage::doDefineTimeOrderFirstIns()
{
	if((mFirstMarketBasePrice < VALUE_ACCURACY) || (mSecondMarketBasePrice < VALUE_ACCURACY) || (mFirstBidMarketPrice < VALUE_ACCURACY) || (mFirstAskMarketPrice < VALUE_ACCURACY))	//保证两腿都收到行情
		return;
	if((mFirstMarketBasePrice > INIT_VALUE) || (mSecondMarketBasePrice > INIT_VALUE) || (mFirstBidMarketPrice > INIT_VALUE) || (mFirstAskMarketPrice > INIT_VALUE))
...
void USTNewConditionArbitrage::doDelOrder(const QString& orderStyle)
{
	if(orderStyle == mOrderLabel){
		mIsDeleted = true;
		if(isInMarket(mFirstInsStatus)){
...
void USTNewConditionArbitrage::submitAction(const QString& insLabel, const QString& orderLocalId, const QString& instrument)
{	
	USTPTradeApi::reqOrderAction(USTPMutexId::getMutexId(), mBrokerId, mUserId, mInvestorId, instrument, orderLocalId, mFrontId, mSessionId);
#ifdef _DEBUG
	QString data = mOrderLabel + QString(tr("  [")) + insLabel + QString(tr("-OrderAction]   UserLocalOrderId: ")) + orderLocalId + QString(tr("  InstrumentId: ")) + instrument;
...
USTPSpeculateOrder::USTPSpeculateOrder(const QString& orderLabel, const QString& speLabel, const QString& firstIns, const QString& secIns, const double& orderPriceTick, const int& qty, const char& bs,  const char& offset,
												 ...
												 :USTPStrategyBase(orderLabel, firstIns, secIns, orderPriceTick, qty, bs, offset, hedge, 0, 0, 0, 0, 0, 0, false, false, false)
{		
	moveToThread(&mStrategyThread);
	mStrategyThread.start();
	mOrderType = 5;
...
USTPStrategyBase(orderLabel, insName, referIns, openPrice, qty, direction, offsetFlag, THOST_FTDC_HF_Speculation, cancelTime, 0, 0, 0, superPriceTick, 0, isAutoCancel, false, isCycle)
{	
	moveToThread(&mStrategyThread);
	mStrategyThread.start();
	mFirstTradeQty = 0;
	...
	mUserId = USTPMutexId::getUserId();
	mInvestorId = USTPMutexId::getInvestorId();
	mFirstSlipPrice = superPriceTick * mFirstInsPriceTick;
