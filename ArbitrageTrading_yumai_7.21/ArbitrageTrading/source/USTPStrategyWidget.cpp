										   ...
										   const int& mdOrderQty, bool isDefineOrder, bool isOpponentPriceType, bool isCancelFirstOrder, bool isCancelSecOrder, bool isCylce, bool isActionReferTick)
{
	...
	QSpinBox *mdOrderQtySpinBox = new QSpinBox();
	mdOrderQtySpinBox->setRange(1, 10000); 
	mdOrderQtySpinBox->setSingleStep(1);
	mdOrderQtySpinBox->setValue(mdOrderQty);
	setCellWidget(nRow, 17, mdOrderQtySpinBox);
...
void USTPStrategyWidget::createNewOrder(const int& row, const double& firstBidPrice, const double& firstAskPrice, const double& secBidPrice, const double& secAskPrice)
{
	...
	QSpinBox* mdOrderQtySpin = qobject_cast<QSpinBox *>(cellWidget(row, 17));
	int mdOrderQty = mdOrderQtySpin->value();
	...
	createStrategy(orderType, firstName, secName, price, oneOpenSlip, secOpenSlip, qty, direction, offstFlag, hedgeFlag, 
		cancelFirstTime, cancelSecTime, cyclePriceStall, actionReferTickNum, actionSuperSlipNum, mdOrderQty, isCancelFirst,
		...
		case 0:
			{
				...
				USTPStrategyBase* pStrategy = new USTPStarePriceArbitrage(label, firstIns, secIns, orderPriceTick, secPriceSlipPoint, qty, bs, 
					offset, hedge, cancelFirstTime, cancelSecTime, cycleStall, mdOrderQty, isAutoFirstCancel, isAutoSecCancel, isCycle, firstBidPrice,
		...
		case 1:
			{
				...
				USTPStrategyBase* pStrategy = new USTPConditionArbitrage(label, firstIns, secIns, orderPriceTick, firstPriceSlipPoint, secPriceSlipPoint, qty, bs, 
					offset, hedge, cancelFirstTime, cancelSecTime, cycleStall,  mdOrderQty, actionReferNum, actionSuperNum, isAutoFirstCancel, isAutoSecCancel, isCycle, isOppentPrice,
		...
		if(isAllTraded && isCycle){
			if((THOST_FTDC_D_Buy == bs) && (THOST_FTDC_OF_Open == offset)){
				...
				createStrategy(orderType, firstIns, secIns, nextOrderPrice, firstPriceSlipPoint, secPriceSlipPoint, qty, THOST_FTDC_D_Sell, nextOffsetFlag, hedge, 
					cancelFirstTime, cancelSecTime, cycleStall, actionReferNum, actionSuperNum, mdOrderQty, isAutoFirstCancel, isAutoSecCancel, isCycle, isOppentPrice,
...
QString USTPStrategyWidget::getList()
{
	...
	for(int row = 0; row < nRowLen; row++){
		...
		QString szLine = firstName + tr("|") + secName + tr("|") + orderType + tr("|") + direction + tr("|") + 
			...
			QString::number(cyclePriceStall) + tr("|") + QString::number(actionReferNum) + tr("|") + QString::number(actionSuperNum) + tr("|") + QString::number(mdOrderQty) +
...
{	
	...
	if(isEnbed){
		doCreateNewOrder(firstIns, secIns, orderStyle, direction, offsetFlag, hedgeFlag, priceTick, orderQty, 0, CANCEL_DELAY_TIME, SUPER_PRICE_STALL,
			SUPER_PRICE_STALL, REFER_PRICE_NUM, 0, 0, REFER_PRICE_NUM, 0, 0, false, false, false, true, false, true);
	}else{
		...
		createStrategy(orderType, firstIns, secIns, priceTick, SUPER_PRICE_STALL, SUPER_PRICE_STALL, orderQty, bs, offstFlag, hFlag, 
			0, CANCEL_DELAY_TIME, 0, REFER_PRICE_NUM, 0, 0, false, true, false, false, false, true, firstMarketBidPrice, firstMarketAskPrice, 
