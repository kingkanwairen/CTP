...
{	
	...
	mSuperWorstPriceLabel = new QLabel(tr("最劣价超价:"));
	mSuperWorstPriceSpinBox = new QDoubleSpinBox;
	mSuperWorstPriceSpinBox->setRange(0.0, 10000.0);
	mSuperWorstPriceSpinBox->setDecimals(3);
	mSuperWorstPriceSpinBox->setSingleStep(0.2);
	mSuperWorstPriceSpinBox->setValue(10.0);
	mSuperWorstPriceSpinBox->setEnabled(false);
	mSuperWorstPriceLabel->setBuddy(mSuperWorstPriceSpinBox);
	...
	mDefineOrderCheckBox = new QCheckBox(tr("定时报单"));
	...
	vBox->addWidget(mSuperWorstPriceLabel, 7, 3, 1, 1);
	vBox->addWidget(mSuperWorstPriceSpinBox, 7, 4, 1, 2);
...
{	
	...
	connect(this, SIGNAL(onCreateNewOrder(const QString&, const QString&, const QString&, const QString&, const QString&, const QString&, const double&, 
		const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const double&, bool, bool, bool, bool, bool, bool)), 
...
void USTParamDialog::doEnableBox(int selIndex)
{
	...
	case 1:
		...
		mSuperWorstPriceSpinBox->setEnabled(true);
		break;
	case 2:
		mOppoentPriceCheckBox->setChecked(false);
		mOppoentPriceCheckBox->setEnabled(true);
...
void USTParamDialog::createOrder()
{
	...
	if(firstName.isEmpty() || secondName.isEmpty()){
		...
		double worstPrice = mSuperWorstPriceSpinBox->value();
		...
		emit onCreateNewOrder(firstName, secondName, style, bs, offset, hedge, price, qty, firtTime, secTime, firstPrice, secondPrice, referTickNum,
			superTickNum, cyclePrice, actionReferTickNum, actionSuperSlipNum, worstPrice, isDefineOrderCheck, isOppoentPriceCheck, isFirstTimeCheck, isSecondTimeCheck,
