...
bool USTPBase64::decodeLicense(const QString& path)
{
	...
	QString qtString =  QString(pEcode.c_str());
	mDecodeLicense <<qtString.split('|');
	file.close();
	return true;
}
...
bool USTPBase64::getDateIsValid(QString& dateTime)
{
	...
	if(current_date.toInt() > 20160830){
		return false;
	}
